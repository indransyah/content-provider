...
class Kreator extends CI_Controller {
    ...
    public function view() {
        ...
        //Pagination
        // $config['base_url'] = base_url('dashboard/kreator/view/');
        // $config['per_page'] = 5;
        ...
        //Title
        $data['title'] = 'Lihat Kreator';

        //Template
        $this->template->backend('kreator', $data);
    }

    public function add() {

        if (!$this->input->post()) {
            $data['title'] = 'Tambah Kreator';
            $this->template->backend('tambah-kreator', $data);
        } else {
